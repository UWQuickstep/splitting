...
void VeloxIn10MinDemo::run() {
  auto row_num = makeFlatVector<int64_t>({0, 1, 2, 3});
  auto c0 = makeFlatVector<int64_t>({0, 1, 0, 1});
  auto c1 = makeFlatVector<int64_t>({0, 0, 1, 0});
  auto key = makeFlatVector<int64_t>({0, 1, 2, 3});
  auto table = makeRowVector({"row_num", "c0", "c1", "key"}, {row_num, c0, c1, key});
  auto plan = PlanBuilder()
                .values({table})
                .window(
                {"min(key) over (PARTITION BY c0, c1 ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) as p0"})
                .project({"row_num", "p0"})
                .planNode();
  auto fact = AssertQueryBuilder(plan).copyResults(pool());

  std::cout << std::endl
            << "Fact table" << fact->toString()
            << std::endl;
  std::cout << fact->toString(0, fact->size()) << std::endl;

  // See aggregations.rst
  plan = PlanBuilder()
          .values({table})
          .aggregation(
            {"c0", "c1"},
            ...
            false)
          .planNode();
  auto dim = AssertQueryBuilder(plan).copyResults(pool());
  std::cout << std::endl
            << "Dim table" << dim->toString()
            << std::endl;
  std::cout << dim->toString(0, dim->size()) << std::endl;
